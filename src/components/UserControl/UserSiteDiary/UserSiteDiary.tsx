...
import AddEditSiteDiary from "./AddEditUserSiteDiary";

import ViewSiteDiaryModal from "./ViewUserSiteDiaryModal";

import { useGetSiteDiaryProjectsQuery } from "../../../features/siteDiary/api/siteDiaryApi";
...
import ConfirmModal from "../../ProjectControl/Project/DeleteModal";
...
} from "../../../features/userSiteDiary/api/userSiteDiaryApi";
...
const UserSiteDiary: React.FC = () => {
  ...
  -----------------------------------*/
  const { data, isLoading, refetch, error } = useGetUserSiteDiariesQuery({
    page,
    limit,
  ...
  /* -----------------------------------
     Delete Logic
  -----------------------------------*/
  const [deleteDiaries] = useDeleteUserSiteDiariesMutation();
  ...
  const getSelectedDiaryData = () => {
    ...
    return rows.map((d) => ({
      ...
      Issues: d.issues || "-",
      [userRole === "USER" ? "Reported To" : "Reported By"]:
        userRole === "USER"
          ? d.reportedTo?.fullName || "—"
          : d.user?.fullName || "—",
            ...
            <table className="min-w-full text-sm border-collapse">
              <thead className="bg-gray-100 text-gray-700">
                <tr className="border-b border-gray-200 text-left text-gray-700 bg-gray-50 whitespace-nowrap">
                  ...
                  {userRole === "USER" && (
                    <th className="p-3 text-center">Reported To</th>
                  )}
