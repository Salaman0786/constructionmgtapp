...
import { X, Sun, CloudRain, CloudSun } from "lucide-react";
import {
...
const AddEditUserSiteDiary: React.FC<Props> = ({
  isOpen,
  onClose,
  diaryId,
}) => {
  ...
  const { data: projectsData, refetch: refetchProjects } =
    useGetUserSiteDiaryProjectsQuery();
  const { data: diaryData, isFetching } = useGetUserSiteDiaryByIdQuery(
    diaryId!,
    {
      skip: !isEdit,
    }
  );
  const [createDiary, { isLoading: creating }] =
    useCreateUserSiteDiaryMutation();
  const [updateDiary, { isLoading: updating }] =
    useUpdateUserSiteDiaryMutation();
  ...
  useEffect(() => {
    ...
    if (isEdit && diaryData?.data?.data?.date) {
      setForm((prev) => ({
        ...prev,
        date: convertToAddisDate(diaryData?.data?.data?.date),
      }));
    }
  ...
  useEffect(() => {
    ...
    const diary = diaryData?.data?.data; // IMPORTANT: Actual diary object
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && diaryId) {
        ...
        showSuccess("User Site diary updated");
      } else {
        await createDiary(payload).unwrap();
        showSuccess("User  Site diary created");
        setForm(initialForm);
        setProjectSearch("");
      }
      onClose();
...
export default AddEditUserSiteDiary;
